    ...
    if load_File is None:
        ...
        while True:
            ...
            if currGameState == Screen.STARTING:
                ...
                outerRect.centerx = WIDTH / 2  
                # draw rectangles at the center of the screen
            ...
            with open(load_file, 'r') as f:
                ...
                game_state['inventory'] = {
                    int(k): v for k, v in game_state['inventory'].items()}
                return currGameState

    def save_objective(currGameState):
        save_date = datetime.now().strftime('%Y-%m-%d-%H-%M-%S')
        with open('objective-{save_date}.save', 'w') as f:
            json.dump(currGameState['tilemap'], f)

    def parse_args(args):
        parser = argparse.ArgumentParser()
        parser.add_argument('-s', '--savegame', dest='savegame',
                            help='The savegame file name to load.')
        parser.add_argument('-o', '--objective', dest='objective',
                            help='The objective for this game.')
        return parser.parse_args(args)

    if __name__ == '__main__':
        args = parse_args(sys.argv[1:]).__dict__
        if 'savegame' in args.keys():
            currGameState = setup_game(load_file=args['savegame'])
        else:
            currGameState = setup_game()
